...
(function (global) {
  'use strict';
  const N = global.CaveNoise;
  const { fbm, ridged, valueNoise, mulberry32 } = N;

  const W = 512, H = 288;            // logical pixel-art resolution (16:9)
  const idx = (x, y) => y * W + x;
  ...
  function makeRenderer(displayCanvas) {
    ...
    const nodePositions = placeNodes(centre, NODES.length, 0.022, 0.90);
  ...
  global.CaveMap = { makeRenderer, W, H };
})(window);
